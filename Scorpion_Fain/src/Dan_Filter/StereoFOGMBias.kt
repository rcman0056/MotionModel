/**
 * Created by Captain Johnson on 11/2/2016.
 * Modified from existing FOGMstate
 */
 ...
 */
class StereoFOGMBias(override var label: String,
                var biasSigma: Double,
                var timeConstant: Double) : DiscreteStateBlock(1, label, mat[1], mat[1]) {


    override var numStates = 1
    private var auxData: FOGM? = null
    ...
    override fun receiveAuxData(auxData: Any) {
        if(auxData is FOGM) {
            biasSigma=auxData.sigma
            timeConstant=auxData.timeconstant
        }
        else
        {
